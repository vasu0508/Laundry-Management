...
public class EditUserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences=getSharedPreferences(getResources().getString(R.string.sharedpref).toString(),MODE_PRIVATE);
        nametext=sharedPreferences.getString("name",null);
        roomnotext=sharedPreferences.getString("roomno", null);
        cardnotext=sharedPreferences.getString("cardno", null);
        deposittext=sharedPreferences.getString("balance", null);
        programtext=sharedPreferences.getString("program", null);
        yeartext=sharedPreferences.getString("year", null);
        emailtext=sharedPreferences.getString("email", null);
        name.setText(nametext);
        email.setText(emailtext);
        program.setText(programtext);
        year.setText(yeartext);
        cardno.setText(cardnotext);
        roomno.setText(roomnotext);
    ...
    private void editDataToSheet(ValueRange body1) {
        ...
        pos=pos+1;
        posit=pos.toString();
        String RANGE = "Sheet1!K"+posit+":T"+posit;
        try {
        ...
        call.enqueue(new Callback<ValueRange>() {
            ...
            public void onResponse(@NonNull Call<ValueRange> call, @NonNull Response<ValueRange> response) {
                ...
                if(entry){
                    ...
                    SendMail sm=new SendMail(EditUserActivity.this,emailtext,"Your Login Credentials for Laundry App","Dear "+nametext+",\n" +
                            "\n" +
                            ...
                            "Laundry Man");
                    sm.execute();
                    Intent intent=new Intent(EditUserActivity.this,SearchUserActivity.class);
            ...
            public void onFailure(@NonNull Call<ValueRange> call, @NonNull Throwable t) {
                Toast.makeText(EditUserActivity.this, "Unable to fetch data", Toast.LENGTH_SHORT).show();
                pb.setVisibility(View.GONE);
                entry=false;
                // Handle error
            }
